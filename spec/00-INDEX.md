...
| CI GitHub Actions (`.github/workflows/build.yml`, pola Presensi: tag `v*` → publish 2 proyek win-x64 self-contained digabung 1 folder → zip → GitHub Release, workflow_dispatch utk build uji manual, cleanup run lama) | Selesai & **dijalankan sungguhan pertama kali 2026-09-08** - tag `v1.0.0` di-push, ketiga job (build/release/cleanup) sukses, `DataMaster-win-x64.zip` (~155 MB) terbit di https://github.com/al-ikhlas86/datamaster-desktop/releases/tag/v1.0.0. |
| Testing menyeluruh 5 kelompok modul (Guru/Siswa/PenugasanMengajar, Kelas/Ekskul, CalonSiswa/KepalaSekolah/User, JadwalPelajaran/Kurikulum, Akademik/KalenderAkademik/TahunAjaran) via curl+DB, dibandingkan langsung ke kode PHP asli | Selesai 2026-09-08. **13 bug nyata ditemukan & diperbaiki** (lihat commit `b2ca35e` dan `dbedbb0`), termasuk 3 bug KRITIS: (1) checkbox "Aktif" form edit Guru tidak pernah bisa menonaktifkan guru (default C#-level menang saat checkbox unchecked tidak terkirim), (2) fitur "Kelola Tingkat" di halaman Kelas mati total 405 (view salah rujuk nama action), (3) perubahan sel Jadwal Pelajaran yang sudah terisi (ganti guru/mapel tanpa tambah/hapus slot lain) hilang diam-diam tanpa error (SaveChanges tidak terpanggil) - plus bug indeks-bentrok-basi dlm 1 file import JadwalPelajaran. Semua modul, termasuk fitur yang sebelumnya belum pernah dites (arsip+alasan keluar Guru, upload/hapus dokumen Siswa, restore-dari-arsip Kelas, hapus peserta Ekskul, DevPreview toggle, Salin Jadwal antar semester/TA), sudah dijalankan &amp; diverifikasi lolos. |
| Uji coba sbg PC TU TK (token Hub API baru, unit_id=2) | Belum dimulai - menunggu instalasi fisik di PC sekolah (lihat `PANDUAN-INSTALASI.md`). |
